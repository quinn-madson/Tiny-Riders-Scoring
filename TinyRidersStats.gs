...
function getTotalLaps(targetRider, targetClass, cacheBuster) {
  ...
  for (var h = 0; h < sheets.length ; h++ ) {
    ...
    totalLaps = totalLaps + processScores(range1, targetRider, targetClass, cacheBuster);
    totalLaps = totalLaps + processScores(range2, targetRider, targetClass, cacheBuster);
  }
...
function processScoresX(range, targetRider, targetClass, cacheBuster) {
  var mode = null;
  var grabLaps = false;
  var rangeLaps = 0;
  var numRows = range.getNumRows();
